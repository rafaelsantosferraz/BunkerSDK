...
//        this.scanCanvasView = scanCanvasView;
    ...
    public void takePicture(){
        autoCapture(ScanHint.CAPTURING_IMAGE);
    }
    ...
    private void scheduleAutoCapture(final ScanHint scanHint) {
        ...
        autoCaptureTimer = new CountDownTimer(500, 100) {
            public void onTick(long millisUntilFinished) {
                if (Math.round((float) millisUntilFinished / 100.0f) != secondsLeft) {
                    secondsLeft = Math.round((float) millisUntilFinished / 100.0f);
                }
                Log.v(TAG, "seconds" + millisUntilFinished / 100);
                switch (secondsLeft) {
                    case 1:
